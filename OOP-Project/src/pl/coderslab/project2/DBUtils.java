...
public class DBUtils {
    ...
    private static final String DB_URL_ToChange = "jdbc:mysql://localhost:3306/dataBaseName?useSSL=false&characterEncoding=utf8&serverTimezone=UTC";
    private static final String DELETE_QUERY = "DELETE FROM tableName WHERE id = ?";
    private static final Scanner scan = new Scanner(System.in);

    public static Connection connect(String dataBaseName) throws SQLException {
        return DriverManager.getConnection(DB_URL_ToChange.replace("dataBaseName", dataBaseName), DB_USER, DB_PASS);
    }
    ...
    public static String getMeString() {
        while (!scan.hasNextLine()) {
            scan.next();
            System.out.println("Wprowadź ciąg literowy");
        }
